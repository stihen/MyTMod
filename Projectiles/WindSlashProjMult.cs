...
{
    public class WindSlashProjMult : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Wind Slash Multiple");
        ...
        public override void AI()
        {
            ...
            // Current implementation of shooting multiple projectiles at different times
            if (projectile.ai[0] < 18)
            {
                projectile.ai[0] += 1;
                if (projectile.ai[0] == 6 || projectile.ai[0] == 12 || projectile.ai[0] == 18)
                {
                    Player player = Main.player[projectile.owner];
                    Random rnd = new Random();
                    int rndX = rnd.Next(-5, 5) * 16;
                    int rndY = rnd.Next(0, 5) * 6;
                    //Dust.CloneDust(180);
                    Projectile.NewProjectile(player.position.X + rndX, player.position.Y - rndY, projectile.velocity.X, projectile.velocity.Y, mod.ProjectileType("WindSlashProj"), projectile.damage, projectile.knockBack, player.whoAmI);
                }
            }
        }
